...
import { RotateCcwIcon, ChevronLeftIcon, ChevronRightIcon, DownloadIcon, RefreshCwIcon, FilesIcon, UndoIcon, RedoIcon } from './icons';
...
interface CanvasProps {
  ...
  canUndo: boolean;
  canRedo: boolean;
  allPoseImages: Record<string, string>;
  isLoading: boolean;
...
const Canvas: React.FC<CanvasProps> = ({ 
  ...
  const handleDownloadAll = () => {
    Object.entries(allPoseImages).forEach(([pose, url]) => {
      // Small delay between downloads to improve browser handling
      setTimeout(() => {
        const link = document.createElement('a');
        link.href = url;
        const filename = pose.replace(/[^a-z0-9]/gi, '_').toLowerCase() + '.png';
        ...
        {displayImageUrl && (
          <>
            ...
            <a 
              ...
              className="flex items-center justify-center text-center bg-white/60 border border-gray-300/80 text-gray-700 font-semibold py-2 px-3 rounded-full transition-all duration-200 ease-in-out hover:bg-white hover:border-gray-400 active:scale-95 text-sm backdrop-blur-sm"
            ...
            <button 
              ...
              disabled={isLoading || Object.keys(allPoseImages).length < 2}
              className="flex items-center justify-center text-center bg-white/60 border border-gray-300/80 text-gray-700 font-semibold py-2 px-3 rounded-full transition-all duration-200 ease-in-out hover:bg-white hover:border-gray-400 active:scale-95 text-sm backdrop-blur-sm disabled:opacity-50 disabled:cursor-not-allowed"
              aria-label="Download All Portraits"
            >
